...
def preprocess_image(image_input) -> np.ndarray:
    ...
    PIPELINE:
      1. Load → grayscale
      2. Auto-crop dark borders (handles video screenshots, letterboxing)
      3. Upscale small images — TrOCR works best on larger images
      4. Deskew — correct tilt
      5. Denoise — reduce scanner grain
      6. Binarize — adaptive threshold

    WHAT CHANGED FROM v1:
      Added auto-border-crop and auto-upscale.
      Border-crop handles video screenshots (black bars on sides).
      Upscale ensures small images (480px) get enough resolution
      for the segmenter to find lines reliably.
    """
    # Accept multiple input types
    if isinstance(image_input, (str, Path)):
        # Handle webp and other formats via PIL first
    ...
    # WHY: Video screenshots, scanned PDFs, and photos often have
    # black/dark borders that confuse contour detection. We find
    # the largest bright region and crop to it.
    gray = _crop_dark_borders(gray)

    # Upscale if image is too small
    # WHY: Images under 800px tall have text lines only 20-30px high.
    # The segmenter needs at least 40px per line to reliably detect them.
    h, w = gray.shape
    if h < 800:
        scale = 800 / h
        gray = cv2.resize(gray, (int(w * scale), 800),
    ...
    # Adaptive binarization — blockSize scaled to image height
    h = denoised.shape[0]
    block_size = max(11, (h // 60) | 1)  # must be odd, scales with image
...
def _crop_dark_borders(gray: np.ndarray,
                       threshold: int = 30,
                       min_crop_ratio: float = 0.03) -> np.ndarray:
    """
    Auto-crop dark borders from document images.

    HOW IT WORKS:
      Finds columns and rows where the mean pixel value is above
      a brightness threshold. Crops to the bounding box of bright content.
      Only crops if the border is at least min_crop_ratio of the dimension
      (avoids unnecessary cropping on clean scans).
    """
    h, w = gray.shape

    # Find rows and columns with meaningful bright content
    ...
    if len(bright_rows) == 0 or len(bright_cols) == 0:
        return gray  # nothing to crop

    top    = int(bright_rows[0])
    bottom = int(bright_rows[-1]) + 1
    left   = int(bright_cols[0])
    right  = int(bright_cols[-1]) + 1

    # Only crop if borders are significant
    if top < h * min_crop_ratio and bottom > h * (1 - min_crop_ratio) and \
       left < w * min_crop_ratio and right > w * (1 - min_crop_ratio):
        return gray  # borders are minimal, skip
...
def _clean_text(text: str) -> str:
    """
    Clean common TrOCR output artifacts.

    WHAT IT FIXES:
      - Trailing periods added by the model: "Hello world ." → "Hello world"
      - Leading/trailing whitespace
      - Multiple spaces → single space
      - Lone punctuation tokens left after stripping

    WHY POST-PROCESS?
      TrOCR occasionally appends a period to lines that don't end with one.
      This is a known artifact of the base model trained on formal documents.
      Simple regex cleanup catches the most common cases without retraining.
    """
    import re
    text = text.strip()
    # Remove trailing space+period artifacts: "word ." → "word"
    text = re.sub(r'\s+\.$', '', text)
    # Remove trailing standalone quotes+period: '" .' → ''
    text = re.sub(r'["\'"]\s*\.$', '', text).strip()
    # Remove leading stray quote characters
    text = re.sub(r'^["\'"]+\s*', '', text)
...
def segment_lines(binary: np.ndarray) -> list:
    ...
    WHAT CHANGED FROM v3:
      Added horizontal ruled line removal.
      Lined paper (notebooks, legal pads, school paper) has horizontal
      lines that after binarization become black horizontal stripes.
      During dilation these connect all text into one massive contour.
      We remove them before dilation using morphological opening.

      Also reduced min_w from 5% to 3% of image width to catch
      shorter text lines and avoid filtering real content.
    """
    img_h, img_w = binary.shape

    # Step 1 — Remove vertical artifacts (page borders, frame lines)
    cleaned = _remove_vertical_lines(binary)

    # Step 2 — Remove horizontal ruled lines (lined paper)
    cleaned = _remove_horizontal_lines(cleaned)

    # Dynamic thresholds
    min_h    = max(8,   int(img_h * 0.012))
    max_h    = min(300, int(img_h * 0.18))
    min_w    = max(20,  int(img_w * 0.03))   # reduced from 0.05
    max_area = img_h * img_w * 0.55

    # Invert: text → white, background → black
    inverted = cv2.bitwise_not(cleaned)

    # Dynamic dilation kernel
    kernel_w = max(20, int(img_w * 0.04))
    ...
    # Merge nearby lines
    line_boxes = _merge_nearby_lines(line_boxes, img_h)

    # Crop each line from ORIGINAL binary (full ink detail for TrOCR)
    lines   = []
    padding = max(4, int(img_h * 0.005))
...
def _remove_vertical_lines(binary: np.ndarray,
                            min_line_length_ratio: float = 0.3) -> np.ndarray:
    """
    Remove vertical black line artifacts from a binarized document image.

    HOW IT WORKS:
      1. Invert the binary (text/lines become white)
      2. Apply morphological opening with a tall vertical kernel
         Opening = erosion then dilation — keeps only structures
         that are tall enough to match the kernel height
      3. The result contains ONLY vertical lines of sufficient height
      4. Subtract these from the original inverted image
      5. Re-invert to get a cleaned binary

    WHY min_line_length_ratio = 0.3?
      A vertical line must span at least 30% of the image height to be
      considered an artifact. This avoids removing tall letters like
      'l', 'I', 'd' which are only a few pixels tall relative to the page.

    RESULT:
      Thick page borders and frame lines are removed.
      Text characters are completely preserved.
    """
    img_h, img_w = binary.shape
    min_length   = int(img_h * min_line_length_ratio)

    # Invert so lines are white on black
    inverted = cv2.bitwise_not(binary)

    # Vertical kernel — tall and narrow
    v_kernel = cv2.getStructuringElement(
        cv2.MORPH_RECT, (1, min_length)
    )
    # Opening detects only structures matching the kernel shape
    vertical_lines = cv2.morphologyEx(inverted, cv2.MORPH_OPEN, v_kernel)

    # Dilate slightly to catch thick lines (1-3px wide)
    dilate_k = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 1))
    vertical_lines = cv2.dilate(vertical_lines, dilate_k, iterations=1)

    # Remove vertical lines from the inverted image
    cleaned_inverted = cv2.subtract(inverted, vertical_lines)

    # Re-invert back to original polarity
    cleaned = cv2.bitwise_not(cleaned_inverted)
    return cleaned


def _remove_horizontal_lines(binary: np.ndarray,
                              min_line_length_ratio: float = 0.4) -> np.ndarray:
    """
    Remove horizontal ruled lines from lined paper documents.

    WHY THIS IS NEEDED:
      Lined paper (notebooks, school paper, legal pads) has horizontal
      lines that binarize to black stripes spanning the full page width.
      During horizontal dilation, these stripes connect all text characters
      into one massive contour covering the entire image height, causing
      the area filter to reject everything.

    HOW IT WORKS:
      Same morphological approach as _remove_vertical_lines but with
      a wide horizontal kernel instead of a tall vertical one.
      Detects and removes lines spanning at least 40% of image width.

    WHY 40%?
      Real ruled lines span 80-100% of the page width.
      The widest individual letters span maybe 5-10%.
      40% is a safe threshold that catches all ruled lines
      while preserving every text character.
    """
    img_h, img_w  = binary.shape
    min_length    = int(img_w * min_line_length_ratio)

    inverted = cv2.bitwise_not(binary)

    # Wide horizontal kernel — detects lines spanning 40%+ of width
    h_kernel = cv2.getStructuringElement(
        cv2.MORPH_RECT, (min_length, 1)
    )
    horizontal_lines = cv2.morphologyEx(
        inverted, cv2.MORPH_OPEN, h_kernel
    )

    # Dilate slightly vertically to catch 2-3px thick lines
    dilate_k = cv2.getStructuringElement(cv2.MORPH_RECT, (1, 3))
    horizontal_lines = cv2.dilate(horizontal_lines, dilate_k, iterations=1)

    # Remove from inverted image
    cleaned_inverted = cv2.subtract(inverted, horizontal_lines)

    return cv2.bitwise_not(cleaned_inverted)


def _merge_nearby_lines(boxes: list, img_h: int,
                        gap_threshold_ratio: float = 0.02) -> list:
    """
    Merge bounding boxes that are vertically very close together.

    WHY THIS IS NEEDED:
      A single handwritten line sometimes produces two separate contours
      due to gaps between words or between ascenders and descenders.
      Without merging, one text line appears as two separate detections
      and TrOCR reads each half-line independently, producing fragments.

    HOW IT WORKS:
      Sort boxes by y. For each box, if the gap to the previous box is
      smaller than gap_threshold (2% of image height), merge them into
      one wider bounding box.
    """
    if not boxes:
        return boxes

    gap = int(img_h * gap_threshold_ratio)
    merged = [boxes[0]]

    for x, y, w, h in boxes[1:]:
        px, py, pw, ph = merged[-1]
        prev_bottom = py + ph
        # If this box starts within gap pixels of the previous box's bottom
        if y - prev_bottom <= gap:
            # Merge: expand previous box to include this one
            new_x = min(px, x)
            new_y = min(py, y)
...
def read_lines(line_images: list, processor: TrOCRProcessor,
    ...
    for i in range(0, len(line_images), INFERENCE_BATCH):
        ...
        # Decode text and clean artifacts
