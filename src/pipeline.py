...
def segment_lines(binary: np.ndarray) -> list:
    ...
    WHAT CHANGED FROM v1:
      Thresholds are now computed dynamically based on image height.
      This makes the segmenter work correctly regardless of whether
      the input is a 480px phone screenshot or a 2400px scanned document.

    DYNAMIC THRESHOLDS:
      MIN_LINE_HEIGHT = 1% of image height  (was fixed 10px)
      MAX_LINE_HEIGHT = 15% of image height (was fixed 200px)
      MIN_LINE_WIDTH  = 5% of image width   (was fixed 50px)
      These ratios were calibrated on documents from 400px to 3000px tall.
    """
    ...
    # Dynamic dilation kernel — wider for larger images
    ...
    # Merge boxes that are very close vertically (same line, split by gap)
    line_boxes = _merge_nearby_lines(line_boxes, img_h)

    # Crop each line from original binary
    lines = []
