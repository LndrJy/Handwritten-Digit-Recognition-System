...
def segment_lines(binary: np.ndarray) -> list:
    ...
    WHAT CHANGED FROM v2:
      Added vertical line removal step before dilation.
      Thick vertical black bars (from photo frames, page borders, scan
      artifacts) were merging all text into one giant contour covering
      the entire image, causing the area filter to reject everything.

      The fix uses morphological opening with a tall vertical kernel to
      DETECT vertical lines, then subtracts them from the binary image
      before running contour detection. This leaves only horizontal
      text content for the segmenter to find.
    """
    img_h, img_w = binary.shape

    # Remove vertical line artifacts
    # WHY: Thick black vertical bars (page borders, photo frames) get
    # dilated into one massive contour that covers the whole image.
    # We detect and remove them before segmentation.
    cleaned = _remove_vertical_lines(binary)

    # Dynamic thresholds based on image dimensions
    min_h    = max(8,   int(img_h * 0.010))
    max_h    = min(300, int(img_h * 0.15))
    min_w    = max(20,  int(img_w * 0.05))
    ...
    kernel_w = max(20, int(img_w * 0.05))
    ...
    # Merge boxes that are very close vertically
    line_boxes = _merge_nearby_lines(line_boxes, img_h)

    # Crop each line from the ORIGINAL binary (not cleaned/dilated)
    # WHY: We use cleaned only to find WHERE lines are.
    # We crop from binary to preserve full ink detail for TrOCR.
    lines   = []
